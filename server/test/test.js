...
describe('Test all api end points', function(){
    ...
    describe('it should get laundry items',() => {
        ...
        beforeEach( done => {
            let sql = `CREATE TABLE IF NOT EXISTS BASE_ITEM(itemid SERIAL, itemame varchar(100),imagePath varchar(500), washingprice integer,category varchar(50), datecreated timestamp);`
            Helper.executeQuery(sql)
            .then(result => {
                let sql = `INSERT INTO BASE_ITEM(itemame,imagePath,washingprice,category, datecreated)
                values
